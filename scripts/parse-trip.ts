...
function matchesName(searchText: string, name: string): boolean {
  const lower = name.toLowerCase()
  if (searchText.includes(lower)) return true
  // Try without " NP" suffix so "Yellowstone NP" matches text containing "Yellowstone"
  ...
  for (const stage of stages) {
    for (const day of stage.days_list) {
      ...
      for (const [name, { coords, type }] of coordEntries) {
        ...
        seen.add(name)
        // Only override type for 'city' entries based on sleep; parks and chargers keep their type
        let finalType = type
        if (type === 'city') {
